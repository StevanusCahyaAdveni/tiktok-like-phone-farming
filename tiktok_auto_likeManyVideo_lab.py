...
class TikTokLikeManyVideosLabU2:
    def __init__(self, root):
        self.root = root
        self.root.title("TikTok Multi-Device Lab - Auto Like, Save & Share Many Videos Engine")
        self.root.geometry("820x880")
        self.is_running = False
        self.active_threads = []
        
        # Thread Locks & Statistik Global
        ...
        self.device_like_counts = {}
        self.device_save_counts = {}
        self.device_share_counts = {}
        
        self.video_save_counts = {}   # {v_idx: int}
        self.video_share_counts = {}  # {v_idx: int}
        self.video_ui_widgets = {}    # {v_idx: {'save_entry': ..., 'share_entry': ...}}
    ...
    def setup_gui(self):
        ...
        tk.Label(frame_target, text="Jumlah Video yang Diputar per Akun:").grid(row=1, column=0, sticky="w", pady=3)
        ...
        btn_generate_video_ui = tk.Button(
            frame_target, 
            text="🔄 Generate / Sesuaikan Form Video", 
            bg="#3498db", 
            fg="white", 
            font=("Arial", 9, "bold"),
            command=self.generate_video_action_forms
        )
        ...
        self.entry_custom_clones.insert(0, "") # Contoh: 1, 2, 3, 11, 12 atau 1-50

        tk.Label(frame_config, text="(Biarkan kosong untuk menggunakan Mulai-Sampai, atau isi misal: 1, 2, 11, 12)", fg="#7f8c8d", font=("Arial", 8, "italic")).grid(row=2, column=1, columnspan=3, sticky="w")
        ...
        # -------------------------------------------------------------
        # FRAME 3: DYNAMIC PER-VIDEO ACTION CONFIGURATION (SIMPAN & SHARE)
        # -------------------------------------------------------------
        self.frame_dynamic_actions = tk.LabelFrame(self.scrollable_content, text=" 3. Konfigurasi Aksi per Video (Simpan/Bookmark & Share/Salin Tautan) ", padx=10, pady=8)
        self.frame_dynamic_actions.pack(fill="x", pady=5)

        self.dynamic_videos_container = tk.Frame(self.frame_dynamic_actions)
        self.dynamic_videos_container.pack(fill="x", expand=True)

        # Inisialisasi awal form video (default 3 video)
        self.generate_video_action_forms()
        ...
        self.btn_start = tk.Button(frame_controls, text="▶ Start Auto-Like & Action Engine", bg="#2ecc71", fg="white", font=("Arial", 11, "bold"), command=self.start_automation)
        ...
        # -------------------------------------------------------------
        frame_stats = tk.LabelFrame(self.scrollable_content, text=" 4. Live Statistics ", padx=10, pady=8)
        frame_stats.pack(fill="x", pady=5)
        
        self.lbl_global_stats = tk.Label(frame_stats, text="Total Clones: 0 | Likes: 0 | Simpan: 0 | Share: 0", font=("Arial", 10, "bold"), fg="#2980b9")
    ...
    def generate_video_action_forms(self):
        """Membuat form konfigurasi aksi dinamis berdasarkan jumlah video yang diinput."""
        try:
        ...
        for v in range(1, total_videos + 1):
            card_frame = tk.LabelFrame(
                ...
                text=f" 🎥 Video #{v} Action Configuration (Auto-Like Aktif) ", 
                font=("Arial", 9, "bold"), 
                fg="#2c3e50", 
                padx=10, 
                pady=6
            ...
            # Target Simpan (Bookmark)
            tk.Label(row_cfg, text="Target Simpan / Bookmark (Akun):").pack(side="left")
            entry_save = tk.Entry(row_cfg, width=10)
            entry_save.pack(side="left", padx=(5, 20))
            entry_save.insert(0, "0")

            # Target Share (Salin Tautan)
            tk.Label(row_cfg, text="Target Share / Salin Tautan (Akun):").pack(side="left")
            entry_share = tk.Entry(row_cfg, width=10)
            entry_share.pack(side="left", padx=(5, 10))
            entry_share.insert(0, "0")

            tk.Label(row_cfg, text="(0 = Nonaktifkan aksi tersebut)", fg="#7f8c8d", font=("Arial", 8, "italic")).pack(side="left", padx=(10, 0))

            self.video_ui_widgets[v] = {
                'save_entry': entry_save,
                'share_entry': entry_share
            }
    ...
    def find_clone_icon_element(self, d, clone_name):
        ...
        for cand in raw_candidates:
            try:
                ...
                # Badge notifikasi biasanya lingkaran kecil (< 75px pada 1080p, atau < 50px pada 720p)
                # atau resource-id mengandung badge / dot / unread / count / notify
                is_badge = False
        ...
        if not valid_candidates:
            # Jika semua elemen yang ditemukan adalah badge, jangan klik! (Artinya app sebenarnya belum terlihat)
            return None
    ...
    def device_worker_thread(self, device_id, target_account, total_videos, watch_delay, clone_names_list, airplane_interval):
        """Worker Thread untuk Auto Like, Save (Bookmark), dan Share (Salin Tautan) Multi-Video per Clone."""
        ...
        try:
            for idx, clone_name in enumerate(clone_names_list, start=1):
                ...
                # 1. Cari ikon aplikasi berdasarkan nama persis (clone_name) dengan filter anti-badge notifikasi
                ...
                # 4. LOOPING MULTI-VIDEO: LIKE, SIMPAN (BOOKMARK), DAN SHARE (SALIN TAUTAN)
                for v_idx in range(1, total_videos + 1):
                    ...
                    # A. DOUBLE-TAP TO LIKE
                    self.logger.log(f"[{device_id}] [Video #{v_idx}] Dispatching Double-Tap to LIKE...")
                    d.double_click(0.5, 0.5, duration=0.04)
                    
                    with self.stats_lock:
                        self.global_liked_videos += 1
                        self.device_like_counts[device_id] = self.device_like_counts.get(device_id, 0) + 1
                    self.update_stats_ui()
                    smart_sleep(1.5)

                    # B. EVALUASI DAN EKSEKUSI SIMPAN / BOOKMARK UNTUK VIDEO v_idx
                    cfg_video = self.video_configs.get(v_idx, {})
                    target_save_count = cfg_video.get('target_save', 0)
                    
                    should_save = False
                    with self.action_lock:
                        curr_save = self.video_save_counts.get(v_idx, 0)
                        if target_save_count > 0 and curr_save < target_save_count:
                            self.video_save_counts[v_idx] = curr_save + 1
                            should_save = True

                    if should_save and self.is_running:
                        self.logger.log(f"[{device_id}] [Video #{v_idx}] Executing SIMPAN / BOOKMARK action...")
                        saved = False
                        
                        bookmark_btn = d(descriptionMatches="(?i).*favorit.*|.*simpan.*|.*bookmark.*|.*save.*")
                        if bookmark_btn.exists(timeout=2):
                            bookmark_btn.click()
                            saved = True
                        else:
                            self.logger.log(f"[{device_id}] Bookmark icon not found by desc, clicking coordinate (0.91, 0.69)...")
                            d.click(0.91, 0.69)
                            saved = True

                        if saved:
                            self.logger.log(f"[{device_id}] [Video #{v_idx}] Video SAVED / BOOKMARKED successfully!")
                            with self.stats_lock:
                                self.global_saved_videos += 1
                                self.device_save_counts[device_id] = self.device_save_counts.get(device_id, 0) + 1
                            self.update_stats_ui()
                            smart_sleep(1.5)

                    # C. EVALUASI DAN EKSEKUSI SHARE (SALIN TAUTAN) UNTUK VIDEO v_idx
                    target_share_count = cfg_video.get('target_share', 0)
                    
                    should_share = False
                    with self.action_lock:
                        curr_share = self.video_share_counts.get(v_idx, 0)
                        if target_share_count > 0 and curr_share < target_share_count:
                            self.video_share_counts[v_idx] = curr_share + 1
                            should_share = True

                    if should_share and self.is_running:
                        self.logger.log(f"[{device_id}] [Video #{v_idx}] Executing SHARE -> SALIN TAUTAN action...")
                        
                        share_btn = d(descriptionMatches="(?i).*bagikan.*|.*share.*")
                        if share_btn.exists(timeout=2):
                            share_btn.click()
                        else:
                            self.logger.log(f"[{device_id}] Share icon not found by desc, clicking coordinate (0.91, 0.77)...")
                            d.click(0.91, 0.77)
                        smart_sleep(2.5)

                        self.logger.log(f"[{device_id}] [Video #{v_idx}] Clicking 'Salin Tautan' button...")
                        ...
                        # TikTok secara otomatis menutup lembar "Bagikan ke" begitu tombol Salin Tautan ditekan.
                        # Jangan tekan tombol back jika panel sudah tertutup agar tidak keluar dari pemutar video!
                        ...
                        if copied:
                            self.logger.log(f"[{device_id}] [Video #{v_idx}] Link COPIED / SHARED successfully!")
                            with self.stats_lock:
                                self.global_shared_videos += 1
                                self.device_share_counts[device_id] = self.device_share_counts.get(device_id, 0) + 1
                            self.update_stats_ui()
                            smart_sleep(1.0)

                    # D. SWIPE KE VIDEO BERIKUTNYA JIKA BELUM VIDEO TERAKHIR
    ...
    def update_stats_ui(self):
        ...
        with self.stats_lock:
            ...
            # Breakdown per video
            vid_stats_list = [
                f"V#{v}: {self.video_save_counts.get(v, 0)} Simpan, {self.video_share_counts.get(v, 0)} Share" 
            ...
            dev_stats_list = [
                f"[{dev}]: {self.device_clone_counts.get(dev, 0)} accs ({self.device_like_counts.get(dev, 0)} likes, {self.device_save_counts.get(dev, 0)} simpan, {self.device_share_counts.get(dev, 0)} share)" 
                for dev in self.device_clone_counts.keys()
            ]
            dev_stats_str = " | ".join(dev_stats_list) if dev_stats_list else "None"
            
        def _update():
            self.lbl_global_stats.config(text=f"Total Clones: {global_c} | Likes: {global_l} | Simpan (Bookmark): {global_s} | Share (Salin Tautan): {global_sh}")
            self.lbl_video_stats.config(text=f"Video Actions: {vid_stats_str}")
    ...
    def start_automation(self):
        ...
        if custom_clones_raw:
            # Parse jika format range '1-50' atau '12-50' atau koma '1, 2, 3, 11, 12'
        ...
        # Parsing konfigurasi dinamis per-video (Simpan & Share)
        self.video_configs = {}
        self.video_save_counts = {}
        self.video_share_counts = {}
        ...
        for v in range(1, total_videos + 1):
            if v in self.video_ui_widgets:
                ...
                try:
                    target_save = int(w['save_entry'].get().strip())
                except ValueError:
                    target_save = 0

                try:
                    target_share = int(w['share_entry'].get().strip())
                except ValueError:
                    target_share = 0
                ...
                self.video_save_counts[v] = 0
                self.video_share_counts[v] = 0
                
                self.logger.log(f"CONFIG Video #{v}: Target Simpan {target_save} akun, Target Share {target_share} akun.")
        ...
        self.device_like_counts = {dev: 0 for dev in devices}
        self.device_save_counts = {dev: 0 for dev in devices}
        self.device_share_counts = {dev: 0 for dev in devices}
        self.update_stats_ui()
        ...
        self.logger.log(f"MASTER CONTROL: Like, Save & Share Engine deployed on {len(devices)} devices ({len(clone_names_list)} accounts: {clone_names_list[:5]}...)...")
...
if __name__ == "__main__":
    root = tk.Tk()
    app = TikTokLikeManyVideosLabU2(root)
    root.mainloop()
