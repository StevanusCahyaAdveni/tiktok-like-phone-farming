...
class TikTokAllInOneLabU2:
    def __init__(self, root):
        self.root = root
        self.root.title("TikTok Multi-Device Lab - All-in-One Engine (Like, Save, Share & Comment)")
    ...
    def setup_gui(self):
        ...
        # -------------------------------------------------------------
        # FRAME 1: TARGET & MULTI-VIDEO CONFIGURATION
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # FRAME 3: DYNAMIC PER-VIDEO ACTION CONFIGURATION (LIKE, SAVE, SHARE, COMMENT)
        # -------------------------------------------------------------
        self.frame_dynamic_actions = tk.LabelFrame(self.scrollable_content, text=" 3. Konfigurasi Aksi per Video (Like, Simpan, Share & Komentar Dinamis) ", padx=10, pady=8)
        ...
        self.btn_start = tk.Button(frame_controls, text="▶ Start All-in-One Engine", bg="#2ecc71", fg="white", font=("Arial", 11, "bold"), command=self.start_automation)
        ...
        self.lbl_global_stats = tk.Label(frame_stats, text="Total Clones: 0 | Likes: 0 | Simpan: 0 | Share: 0 | Comments: 0", font=("Arial", 10, "bold"), fg="#2980b9")
    ...
    def generate_video_action_forms(self):
        ...
        for v in range(1, total_videos + 1):
            card_frame = tk.LabelFrame(
                ...
                text=f" 🎥 Video #{v} Configuration (Auto-Like Selalu Aktif) ", 
                font=("Arial", 9, "bold"), 
                fg="#2c3e50", 
                padx=10, 
                pady=6
            ...
            # Baris 1: Target Simpan & Target Share
            row_actions = tk.Frame(card_frame)
            row_actions.pack(fill="x", pady=2)

            tk.Label(row_actions, text="Target Simpan / Bookmark:").pack(side="left")
            entry_save = tk.Entry(row_actions, width=8)
            entry_save.pack(side="left", padx=(5, 15))
            entry_save.insert(0, "0")

            tk.Label(row_actions, text="Target Share / Salin Tautan:").pack(side="left")
            entry_share = tk.Entry(row_actions, width=8)
            entry_share.pack(side="left", padx=(5, 15))
            entry_share.insert(0, "0")

            tk.Label(row_actions, text="(0 = Nonaktifkan Simpan/Share)", fg="#7f8c8d", font=("Arial", 8, "italic")).pack(side="left")

            # Baris 2: Start Account Komen & Target Jumlah Komen
            row_comment_cfg = tk.Frame(card_frame)
            row_comment_cfg.pack(fill="x", pady=(4, 2))

            tk.Label(row_comment_cfg, text="Mulai Komen dari Akun #:").pack(side="left")
            entry_start_acc = tk.Entry(row_comment_cfg, width=8)
            entry_start_acc.pack(side="left", padx=(5, 15))
            entry_start_acc.insert(0, "1")

            tk.Label(row_comment_cfg, text="Target Jumlah Komentar:").pack(side="left")
            entry_target_cmt = tk.Entry(row_comment_cfg, width=8)
            entry_target_cmt.pack(side="left", padx=(5, 15))
            entry_target_cmt.insert(0, "0")

            tk.Label(row_comment_cfg, text="(0 = Auto sesuai jumlah baris di bawah / Kosongkan jika tanpa komen)", fg="#7f8c8d", font=("Arial", 8, "italic")).pack(side="left")

            # Baris 3: List Komentar Text Box
    ...
    def device_worker_thread(self, device_id, target_account, total_videos, watch_delay, clone_names_list, airplane_interval):
        """Worker Thread untuk All-in-One Engine (Like, Save, Share & Comment Multi-Video per Clone)."""
        ...
        try:
            for idx, clone_name in enumerate(clone_names_list, start=1):
                ...
                # 4. LOOPING MULTI-VIDEO: LIKE, SIMPAN, SHARE, DAN KOMENTAR PER VIDEO
                for v_idx in range(1, total_videos + 1):
                    ...
                    # D. EVALUASI DAN EKSEKUSI KOMENTAR UNTUK VIDEO v_idx
                    ...
                    # E. SWIPE KE VIDEO BERIKUTNYA JIKA BELUM VIDEO TERAKHIR
    ...
    def update_stats_ui(self):
        ...
        with self.stats_lock:
            ...
            # Breakdown per video
            vid_stats_list = [
                f"V#{v}: {self.video_save_counts.get(v, 0)} S, {self.video_share_counts.get(v, 0)} Sh, {self.video_comment_counts.get(v, 0)} C" 
            ...
            dev_stats_list = [
                f"[{dev}]: {self.device_clone_counts.get(dev, 0)} accs ({self.device_like_counts.get(dev, 0)}L, {self.device_save_counts.get(dev, 0)}S, {self.device_share_counts.get(dev, 0)}Sh, {self.device_comment_counts.get(dev, 0)}C)" 
                for dev in self.device_clone_counts.keys()
            ]
            dev_stats_str = " | ".join(dev_stats_list) if dev_stats_list else "None"
            
        def _update():
            self.lbl_global_stats.config(text=f"Total Clones: {global_c} | Likes: {global_l} | Simpan: {global_s} | Share: {global_sh} | Komentar: {global_cmt}")
    ...
    def start_automation(self):
        ...
        # Parsing konfigurasi dinamis per-video (Simpan, Share, & Komentar)
        self.video_configs = {}
        ...
        for v in range(1, total_videos + 1):
            if v in self.video_ui_widgets:
                ...
                self.logger.log(f"CONFIG Video #{v}: Target Simpan {target_save}, Target Share {target_share}, Komen Start #{start_acc} (Target {target_cmt}).")
        ...
        self.logger.log(f"MASTER CONTROL: All-in-One Engine deployed on {len(devices)} devices ({len(clone_names_list)} accounts: {clone_names_list[:5]}...)...")
